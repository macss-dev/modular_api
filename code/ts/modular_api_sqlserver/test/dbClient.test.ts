...
describe('DbConnectionSettings', () => {
  it('normalizes environment defaults and redacts secrets', () => {
    ...
    expect(settings.engineId).toBe('sqlserver');
    expect(settings.host).toBe('db.local');
    expect(settings.port).toBe(14333);
    expect(settings.database).toBe('modular_api_graphql_v1');
    expect(settings.username).toBe('sa');
    expect(settings.password).toBe('super-secret');
    expect(settings.redactedSummary).toContain('db.local:14333');
    expect(settings.redactedSummary).toContain('sa@');
...
describe('DbClient', () => {
  ...
  it('describes its provider and closes cleanly', async () => {
    ...
    expect(client.describe().engineId).toBe('sqlserver');
