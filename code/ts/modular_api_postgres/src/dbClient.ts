...
export class DbConnectionSettings {
  ...
  public static fromEnvironment(
    ...
    const parsedPort = Number.parseInt(environment.MODULAR_API_POSTGRES_PORT ?? '', 10);

    return new DbConnectionSettings({
      host: environment.MODULAR_API_POSTGRES_HOST ?? '127.0.0.1',
      port: Number.isNaN(parsedPort) ? 5432 : parsedPort,
      database: environment.MODULAR_API_POSTGRES_DATABASE ?? 'modular_api_graphql_v1',
      username: environment.MODULAR_API_POSTGRES_USERNAME ?? 'postgres',
      password: environment.MODULAR_API_POSTGRES_PASSWORD ?? 'postgres',
      sslMode: environment.MODULAR_API_POSTGRES_SSLMODE ?? 'disable',
    });
  }
  ...
  public get redactedSummary(): string {
    return `${this.engineId}://${this.username}@${this.host}:${this.port}/${this.database}?sslmode=${this.sslMode}`;
  }
}
