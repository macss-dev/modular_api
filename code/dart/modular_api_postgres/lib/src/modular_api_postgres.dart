...
final class DbConnectionSettings {
  ...
  factory DbConnectionSettings.fromEnvironment({
    ...
    final parsedPort = int.tryParse(env['MODULAR_API_POSTGRES_PORT'] ?? '');

    return DbConnectionSettings(
      host: env['MODULAR_API_POSTGRES_HOST'] ?? '127.0.0.1',
      port: parsedPort ?? 5432,
      database: env['MODULAR_API_POSTGRES_DATABASE'] ??
          'modular_api_graphql_v1',
      username: env['MODULAR_API_POSTGRES_USERNAME'] ?? 'postgres',
      password: env['MODULAR_API_POSTGRES_PASSWORD'] ?? 'postgres',
      sslMode: env['MODULAR_API_POSTGRES_SSLMODE'] ?? 'disable',
    );
  }
  ...
  String get engineId => 'postgres';

  String get redactedSummary {
    return '$engineId://$username@$host:$port/$database?sslmode=$sslMode';
  }
}
